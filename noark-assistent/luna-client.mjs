...
import { MODEL_ID, cleanConversation } from './rag-shared.mjs';
...
export async function loadLunaStatus() {
  try {
    ...
    if (status.model !== MODEL_ID || status.corpusVersion !== BUILD_INFO.corpusVersion)
      throw new Error('Backend og kildebase må oppdateres samtidig.');
    return { ...status, configured: status.configured === true && typeof status.siteKey === 'string' && Boolean(status.siteKey),
      message: status.configured ? 'Luna-backend er klar. Aktiver KI nedenfor.' : 'Luna venter på sikker aktivering. Lokalt kildesøk fungerer.' };
...
export function validateUiAnswer(answer) {
  if (!answer || answer.mode !== 'luna' || answer.model !== MODEL_ID || answer.corpusVersion !== BUILD_INFO.corpusVersion ||
  ...
  const results = answer.results.map((r, i) => {
    ...
    if (!record || ids.has(record.id) || r.rank !== i + 1 || !Number.isInteger(r.relevance) || r.relevance < 0 || r.relevance > 100 ||
...
export async function askLuna(question, history, token, signal) {
