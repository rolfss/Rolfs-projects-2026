export class AudioEngine {
  ...
  startAmbient(stage = 1) {
    ...
    this.stage = stage;
    const fundamentals = [45, 49.5, 46, 42, 38.5, 36, 33, 29.5];
    ...
    for (const [ratio, gain] of [[1, 0.04], [1.5, 0.018], [2, 0.009]]) {
      ...
      filter.type = 'lowpass';
      filter.frequency.value = 290 + stage * 62;
      filter.Q.value = 0.8;
  ...
  schedulePulse() {
    ...
    const intervals = [2050, 1880, 1660, 1480, 1320, 1180, 1040, 900];
    const delay = intervals[this.stage - 1] || intervals[0];
    this.pulseTimer = window.setTimeout(() => {
      if (this.enabled && this.ambientNodes.length) {
        const roots = [147, 165, 156, 139, 131, 123, 110, 98];
  ...
  setStage(stage) {
    this.stage = Math.max(1, Math.min(8, Number(stage) || 1));
    const context = this.ensure();
    if (!context || !this.ambientNodes.length) return;
    const base = [45, 49.5, 46, 42, 38.5, 36, 33, 29.5][this.stage - 1];
    const now = context.currentTime;
    for (const node of this.ambientNodes) {
      ...
      node.filter.frequency.linearRampToValueAtTime(290 + this.stage * 62, now + 0.7);
    }
    this.schedulePulse();
  }
  ...
  stageUp(stage) {
    const base = [220, 247, 233, 196, 185, 165, 147, 131][stage - 1] || 220;
  ...
  perkOpen() {
    this.tone(196, 0.11, { type: 'sine', gain: 0.018, pan: -0.35 });
    this.tone(294, 0.14, { type: 'triangle', gain: 0.022, delay: 0.08, pan: 0.35 });
  }

  perk() {
    [330, 440, 660].forEach((frequency, index) => this.tone(frequency, 0.15, {
      type: 'triangle', gain: 0.024, delay: index * 0.065, pan: -0.25 + index * 0.25,
    }));
  }

  automation() {
    [520, 740, 980].forEach((frequency, index) => this.tone(frequency, 0.08, {
      type: 'square', gain: 0.018, delay: index * 0.045, pan: 0.35 - index * 0.35,
    }));
  }

  overtime() {
    this.tone(176, 0.18, { type: 'sawtooth', gain: 0.025, endFrequency: 132 });
    this.tone(352, 0.11, { type: 'square', gain: 0.018, delay: 0.19 });
    this.tone(352, 0.11, { type: 'square', gain: 0.018, delay: 0.38 });
  }

  timeout() {
    [196, 174, 147, 131].forEach((frequency, index) => this.tone(frequency, 0.19, {
      type: 'triangle', gain: 0.021, delay: index * 0.11,
    }));
  }

  careerUp() {
    [392, 523, 659, 784, 1047].forEach((frequency, index) => this.tone(frequency, 0.17, {
      type: 'triangle', gain: 0.023, delay: index * 0.065, pan: Math.sin(index) * 0.25,
    }));
  }
