...
Use --background --factory-startup --python this-file -- [--gpu] [--size 512].
...
scene.cycles.use_denoising=True;scene.cycles.seed=26
...
brass=mat('Brushed brass',(.48,.31,.11),.78,.34)
steel=mat('Archive steel',(.025,.055,.052),.4,.48)
...
for side in [-1,1]:
    ...
    box('Upper sill',side*10,6.3,22,.7,.4,60,stone)
    box('Cornice proxy',side*10,11.8,22,.8,.4,60,stone)
    for z in [-3,3,15,21,33,39,51]:
        ...
        box('Column foot',side*7.6,.15,z,1.3,.3,1.3,stone)
    box('Roof proxy',side*6.7,16.4,22,6.6,.25,60,stone)
box('Rear wall',0,8,54,20,16,.65,stone)
box('Memory plinth',0,.5,20,1.65,1,1.6,stone)
scene.world.use_nodes=True
scene.world.node_tree.nodes['Background'].inputs['Color'].default_value=(.60,.69,.81,1)
...
for kind in ('ao','indirect'):
    image=bpy.data.images.new('Hall '+kind,width=a.size,height=a.size*3,alpha=False)
    ...
    image.filepath_raw=str(out/f'hall-{kind}.png');image.file_format='PNG';image.save()
    stone.node_tree.nodes.remove(node)
bpy.ops.wm.save_as_mainfile(filepath=str(out/'hall-lighting.blend'))
bpy.ops.object.select_all(action='SELECT');bpy.ops.object.delete(use_global=False)
objects=[]
def part(*args,**kwargs):
    o=box(*args,**kwargs);objects.append(o);return o
part('Rack shell',-.95,1.62,0,1.30,2.65,.87,steel,.045)
part('Rack crown',-.95,2.98,0,1.36,.08,.93,brass,.018)
for i in range(9):
    y=.50+i*.265
    part('Drive tray',-.95,y,.47,1.18,.21,.08,oak,.012)
    part('Drive grip',-.95,y+.025,.53,.38,.032,.035,brass,.009)
    for dx in [-.48,.48]:part('Fastener',-.95+dx,y,.524,.035,.035,.012,brass,.006)
    for j in range(5):part('Vent slot',-1.34+j*.085,y-.065,.515,.05,.012,.012,steel)
part('Reading desk',.92,1.10,0,1.50,.14,1.15,oak,.035)
for x in [.33,1.51]:
    for z in [-.43,.43]:part('Desk leg',x,.64,z,.055,.88,.055,brass,.012)
part('Display housing',.92,1.77,-.24,1.35,1.06,.095,brass,.025)
part('Display face',.92,1.77,-.181,1.25,.96,.016,black,.008)
part('Reading shelf',.92,1.24,.23,1.20,.045,.53,paper,.01)
for x in [.48,.77,1.06,1.35]:part('Folder divider',x,1.37,.27,.22,.20,.40,paper,.008)
part('Connection rail',0,.34,.7,3.3,.035,.035,brass,.008)
for m in (oak,brass,steel,paper,black):
    bpy.ops.object.select_all(action='DESELECT')
    chosen=[o for o in objects if o.name in bpy.data.objects and o.data.materials[0]==m]
