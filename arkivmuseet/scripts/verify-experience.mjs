...
const check=(value,message)=>{assert.ok(value,message);checks.push(message);console.log(`✓ ${message}`);};
// Pause the already-rendered canvas during capture so software WebGL does not
// starve Chromium's screenshot compositor on a shared CI runner.
...
async function enter(flat=true){await page.goto(url,{waitUntil:'domcontentloaded',timeout:90000});if(flat)await page.locator('#flat-enter').click();else{await page.locator('#guided:not([disabled])').waitFor({timeout:90000});await page.locator('#guided').click();}if(await page.locator('#gallery-next').count())await page.locator('#gallery-next').click();await page.locator('#mission-title').waitFor();}
...
try{
 ...
 await enter();await snap('01-room-story-flat');
 ...
 for(const [index,s] of roomStories.entries()){
  ...
  for(const [i,id] of s.solution.entries()){
   ...
   if(s.id==='osen'&&id==='search')await page.locator('#journal-query').press('Enter');else {await page.locator(`[data-tool="${id}"]`).focus();await page.keyboard.press('Enter');}check((await progress(s.id)).investigation.length===i+1,`${s.id}: visual probe ${id} recorded`);
 ...
 await page.locator(`[data-final-answer="${finalQuiz[0].answer}"]`).click();await page.locator(`[data-final-answer="${finalQuiz[1].answer}"]`).click();check((await page.locator('#mission-title').innerText()).includes('løfte'),'Final synthesis completes the six-room journey');await snap('finale');const completedSave=await page.evaluate(()=>localStorage.getItem('arkivmuseet-narrative-v2'));
 ...
 // Reuse state produced by the real play-through as a visual fixture. The
 // functional checks above do not depend on graphics acceleration.
 const visual=await browser.newContext({viewport:{width:1440,height:1000},reducedMotion:'reduce'});
 page=await visual.newPage();page.on('pageerror',e=>errors.push(e.message));
 const fixture=JSON.parse(completedSave);for(const p of Object.values(fixture.rooms))p.phase=1;
 await page.addInitScript(saved=>{localStorage.setItem('arkivmuseet-narrative-v2',JSON.stringify(saved));localStorage.setItem('arkivmuseet-preferences',JSON.stringify({reduced:true,quality:.75}));},fixture);
 await enter(false);
 check((await page.evaluate(()=>window.museumDiagnostics())).triangles>30000,'3D museum renders actual geometry');
 for(const s of roomStories){
  await openRoom(s.id);
  await page.waitForFunction(id=>window.museumDiagnostics().chapters.some(c=>c.id==='chapter-installation-'+id&&c.stage?.steps>0),s.id,{timeout:30000});
  const frame=await page.evaluate(()=>window.museumDiagnostics().frameCount);await page.waitForFunction(n=>window.museumDiagnostics().frameCount>n,frame);
  check(true,`${s.id}: physical 3D exhibit receives the completed investigation`);
  await page.locator('.chapter-scene').scrollIntoViewIfNeeded();await snap(`3d-${s.id}`);
 }
 await writeFile(out+'/3d-diagnostics.json',JSON.stringify(await page.evaluate(()=>window.museumDiagnostics()),null,2));
