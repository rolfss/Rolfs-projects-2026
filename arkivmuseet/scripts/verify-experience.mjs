...
const snap=async(name)=>{const path=`${out}/${name}.png`;await page.screenshot({path,fullPage:false});shots.push(path);};
...
try{
 ...
 browser=await chromium.launch({executablePath:process.env.MUSEUM_CHROME||undefined,headless:true,args:['--enable-webgl','--ignore-gpu-blocklist','--use-angle=swiftshader','--enable-unsafe-swiftshader']});
 ...
 await enter();check((await page.evaluate(()=>window.museumDiagnostics())).triangles>30000,'3D museum renders actual geometry');await snap('01-room-story');
 ...
 for(const size of [{width:390,height:844},{width:320,height:568},{width:800,height:700}]){
  await page.setViewportSize(size);await page.evaluate(()=>{localStorage.removeItem('arkivmuseet-narrative-v2');localStorage.setItem('arkivmuseet-preferences',JSON.stringify({reduced:true,textsize:'1.3'}));});await enter(true);await page.locator('#step-next').click();await noOverflow(`Flat ${size.width}px / 130% text`);await page.locator('[data-tool="metadata"]').click();check((await progress('osen')).investigation.length===1,`Keyboard/touch-sized controls operate at ${size.width}px`);await page.locator('.chapter-scene').scrollIntoViewIfNeeded();await snap(`mobile-${size.width}`);
 }
...
}catch(e){errors.push(String(e));if(page)try{await snap('FAILURE');await writeFile(out+'/failure-page.txt',await page.locator('body').innerText());}catch{}process.exitCode=1;}
