...
async function openInvestigation(p){await p.locator('#menu-open').click();await p.locator('#menu-investigation').click();}
async function visitControl(p,id){if(await p.locator('#visit-details').isHidden())await p.locator('#visit-toggle').click();await p.locator(id).click();}
...
try{
 ...
 await openInvestigation(page);await action(page,'start').click();
 ...
 await openInvestigation(page);await action(page,'crisis').click();
 ...
 await page.locator('#enter').click();await openInvestigation(page);await action(page,'room','osen').click();await action(page,'real','osen').click();
 ...
 await visitControl(page,'#home');await page.screenshot({path:out+'/06-hall-restored.png'});
 ...
 mobile.on('pageerror',e=>errors.push('mobile: '+e.message));await enter(mobile);await openInvestigation(mobile);await action(mobile,'start').click();
 ...
 await privatePage.goto(url);await privatePage.locator('#flat-enter').click();await openInvestigation(privatePage);check(await privatePage.locator('.case17-warning').isVisible(),'Storage failure is disclosed without blocking play');await privatePage.close();
 const text=await openPage({javaScriptEnabled:false,viewport:{width:390,height:844}});const textResponse=await text.goto(url+'etterforskning.html');check(textResponse.ok(),'No-JavaScript investigation text is deployed');check(await text.locator('h1').innerText()==='Det manglende grunnlaget','The text title explains the investigation');check(await text.locator('article').count()===26,'All 10 evidence, 6 triage, 4 access and 6 decision items have a text equivalent');
