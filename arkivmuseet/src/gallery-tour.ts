...
export class GalleryTour{
  ...
  private render(){
    ...
    this.host.innerHTML=`<div class="gallery-flat-art" aria-hidden="${item?'true':'false'}">${!item?`<div class="gallery-flat-benefit">${benefitCard(c.id)}</div>`:image?`<img src="${import.meta.env.BASE_URL+esc(image)}" alt="">`:`<div class="gallery-flat-panel"><small>${esc(item?.kicker??'ARKIVMUSEET')}</small><p>${esc(item?.title??room.name)}</p><span>${esc(item?.text??room.introduction)}</span></div>`}</div><div class="gallery-plaque ${!item?'benefit-overview':''}"><div class="gallery-heading"><p class="eyebrow">ROM ${number} / 5 · ${item?`VERK ${this.index+1} / ${items.length}`:'VELKOMMEN INN'}</p><button id="gallery-free" aria-label="Lukk omvisningen og gå fritt">Gå fritt ↗</button></div><h2 tabindex="-1">${esc(item?.title??room.name)}</h2>${!item?benefitCard(c.id):''}<p class="gallery-caption">${esc(item?(item.kind==='photo'?item.caption:item.text):room.introduction)}</p>${item?`<button id="gallery-detail" class="gallery-source">${item.image?'Se bildet i stort format':'Les hele veggpanelet'} · Kilde og bildetekst ↗</button>`:`<p class="gallery-invitation">Historien, øvelsen og romprøven hører sammen. Veggbildene kan utforskes før eller etterpå.</p>`}<nav class="gallery-navigation" aria-label="Omvisning"><button id="gallery-prev" aria-label="Forrige verk" ${this.index<0?'disabled':''}>←</button><button id="gallery-next" class="primary">${this.index<0?'Start rommets historie':this.index<items.length-1?'Neste verk':'Undersøk saken selv'} <span>→</span></button><button id="gallery-mission">${this.index<0?'Se veggbildene først ↗':'Til romoppdraget ↗'}</button></nav><p class="gallery-footer">${esc(item?item.kicker:room.name)}${imageItem?.image&&item?` · ${esc(imageItem.credit)}`:''}</p></div>`;
