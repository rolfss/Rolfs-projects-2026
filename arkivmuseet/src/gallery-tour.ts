...
export class GalleryTour{
  ...
  private render(){
    ...
    this.host.innerHTML=`<div class="gallery-flat-art" aria-hidden="true">${image?`<img src="${import.meta.env.BASE_URL+esc(image)}" alt="">`:`<div class="gallery-flat-panel"><small>${esc(item?.kicker??'ARKIVMUSEET')}</small><p>${esc(item?.title??room.name)}</p><span>${esc(item?.text??room.introduction)}</span></div>`}</div><div class="gallery-plaque"><div class="gallery-heading"><p class="eyebrow">ROM ${number} / 5 · ${item?`VERK ${this.index+1} / ${items.length}`:'VELKOMMEN INN'}</p><button id="gallery-free" aria-label="Lukk omvisningen og gå fritt">Gå fritt ↗</button></div><h2 tabindex="-1">${esc(item?.title??room.name)}</h2><p class="gallery-caption">${esc(item?(item.kind==='photo'?item.caption:item.text):room.introduction)}</p>${item?`<button id="gallery-detail" class="gallery-source">${item.image?'Se bildet i stort format':'Les hele veggpanelet'} · Kilde og bildetekst ↗</button>`:`<p class="gallery-invitation">Se deg rundt. Ta inn rommet. Gå videre når du er klar.</p>`}<nav class="gallery-navigation" aria-label="Omvisning"><button id="gallery-prev" aria-label="Forrige verk" ${this.index<0?'disabled':''}>←</button><button id="gallery-next" class="primary">${this.index<0?'Se første verk':this.index<items.length-1?'Neste verk':number<5?'Neste rom':'Til lederens rom'} <span>→</span></button><button id="gallery-mission">Prøv lederoppdraget ↗</button></nav><p class="gallery-footer">${esc(item?item.kicker:room.name)}${imageItem?.image&&item?` · ${esc(imageItem.credit)}`:''}</p></div>`;
    this.host.querySelector<HTMLButtonElement>('#gallery-prev')!.onclick=()=>this.move(this.index-1);
    this.host.querySelector<HTMLButtonElement>('#gallery-next')!.onclick=()=>{if(this.index<items.length-1)this.move(this.index+1);else this.hooks.next(this.cases[number]?.id??'leader');};
    this.host.querySelector<HTMLButtonElement>('#gallery-free')!.onclick=()=>this.hooks.free();
    this.host.querySelector<HTMLButtonElement>('#gallery-mission')!.onclick=()=>this.hooks.mission(c.id);
