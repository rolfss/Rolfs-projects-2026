...
export class RoomStoryWorld{
 ...
 build(id:string){if(this.boards.has(id))return;const c=this.cases.find(c=>c.id===id);if(!c)return;
  ...
  const frame=new T.Mesh(new T.BoxGeometry(5.18,2.94,.16),new T.MeshStandardMaterial({color:'#a78c50',metalness:.55,roughness:.4}));frame.position.set(0,3.5,-.64);group.add(frame);
  const mesh=new T.Mesh(new T.PlaneGeometry(5.04,2.772),new T.MeshBasicMaterial({color:0xffffff,toneMapped:false}));mesh.name='chapter-screen-'+id;mesh.position.set(0,3.5,-.55);group.add(mesh);
