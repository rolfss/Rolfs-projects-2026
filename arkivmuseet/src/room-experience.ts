...
export function renderRoom(j:Journey,c:MuseumCase,focus=false){
 ...
 if(p.phase===0){
  body=`<p class="case-meta">${esc(c.organization)} · ${esc(c.eventDate)} · Historisk sak</p><h2 id="mission-title" tabindex="-1">${esc(s.title)}</h2><p class="story-prose">${esc(s.opening)}</p><div class="room-problem"><strong>Det som sviktet</strong><p>${esc(s.problem)}</p></div>${scene}${source}<p class="room-boundary">${esc(c.consequence[0].text)}</p><p class="room-contract">Du skal undersøke et spor, prøve et ledervalg og svare på to spørsmål. Da får du rommets læringsmerke.</p>`;
  ...
  body=`<div class="earned-emblem" aria-hidden="true">✓</div><p class="simulation-label">LÆRINGSMERKE ${j.count()} / 5</p><h2 id="mission-title" tabindex="-1">${esc(s.badge)}</h2>${moral}${stampStrip(earned(j))}<p class="story-prose">${j.count()===5?'Alle fem rom er løst. Nå kan du åpne sluttrommet og bruke lærdommene sammen.':esc(s.bridge)}</p><div class="takeaway"><h3>Ta det med tilbake</h3><p>${esc(s.transfer)}</p><button id="room-add">${j.state.plan[c.id]?.selected?'✓ Lagt i lederbestillingen':'Legg til min lederbestilling + '}</button></div>`;
