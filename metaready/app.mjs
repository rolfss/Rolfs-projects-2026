...
function brief(id){const asset=state.assets.find(a=>a.id===id)||state.assets[0];const findings=validateAsset(asset,state.assets,relationships);const quality=calculateQuality(asset,findings);const assessment=assessReadiness(asset,state.useCaseId);const related=state.backlog.filter(b=>b.assetIds.includes(asset.id));download(`${asset.id.toLowerCase()}-governance-brief.md`,createGovernanceBrief(asset,findings,quality,assessment,impacted(asset.id),related),'text/markdown;charset=utf-8');addAudit('Eksporterte styringsbrief',asset.title,`Brief for ${assessment.useCase.label} ble generert lokalt.`);save();toast('Styringsbrief eksportert.');}
...
function exportBacklog(){const rows=[['Priority','Title','Assets','Owner role','Status','Reason'],...state.backlog.map(x=>[priorityScore(x),x.title,x.assetIds.join('; '),x.ownerRole,x.status,x.reason])];download('metaready-remediation-backlog.csv',rows.map(r=>r.map(csvCell).join(',')).join('\n'),'text/csv;charset=utf-8');toast('Tiltaksloggen er eksportert.');}
...
function workflow(id,next){if(!require('workflow'))return;const asset=state.assets.find(a=>a.id===id);const before=asset.status;asset.status=next;asset.version=asset.version.replace(/\d+$/,String(Number(asset.version.split('.').at(-1)||0)+1));addAudit(`Endret arbeidsflyt: ${before} → ${next}`,asset.title,`Status ble endret av ${roleName(state.role)}.`);save();render();toast(`Status endret til ${next}.`);}
...
function register(fd){if(!require('register'))return;const id=`CW-${String(fd.get('type')).startsWith('Data service')?'API':'NEW'}-${String(state.assets.length+1).padStart(3,'0')}`;const asset={id,title:fd.get('title'),type:fd.get('type'),unit:'New registration',description:fd.get('description'),owner:fd.get('owner'),steward:fd.get('steward'),status:'Draft',sensitivity:fd.get('sensitivity'),provenance:fd.get('provenance'),accessRights:fd.get('accessRights'),retention:fd.get('retention'),updateFrequency:fd.get('updateFrequency'),reviewDate:fd.get('reviewDate'),version:'0.1.0',glossaryCoverage:0,qualityEvidence:'',lawfulBasis:'',contact:'',aiAllowed:false,machineReadable:false,identifiersStable:true,documentationValue:'Unassessed'};state.assets.unshift(asset);state.selectedAssetId=id;state.activeView='catalog';state.detailOpen=true;addAudit('Registrerte informasjonsressurs',asset.title,`${asset.id} ble opprettet som utkast og automatisk validert.`);save();dialog.close();form.reset();render();toast('Ressurs registrert og validert.');}
...
document.addEventListener('click',event=>{
  ...
  if(action==='record-impact'){const asset=state.assets.find(a=>a.id===el.dataset.id);addAudit('Registrerte konsekvensbeslutning',asset.title,'Berørte eiere skal konsulteres før vesentlig endring publiseres.');save();toast('Beslutning registrert i styringssporet.');}
  if(action==='export-backlog')exportBacklog();
  if(action==='reset-demo'&&confirm('Nullstille lokale endringer og gå tilbake til syntetiske eksempeldata?')){state=defaults();localStorage.removeItem(KEY);role.value=state.role;render();toast('Demoen er nullstilt.');}
});
document.addEventListener('change',event=>{
  ...
  if(el.dataset.action==='backlog-status'){if(!require('remediate')){render();return;}const item=state.backlog.find(x=>x.id===el.dataset.id);const before=item.status;item.status=el.value;addAudit(`Endret tiltak: ${before} → ${item.status}`,item.title,`Tiltaksstatus oppdatert av ${roleName(state.role)}.`);save();toast('Tiltaksstatus oppdatert.');}
});
