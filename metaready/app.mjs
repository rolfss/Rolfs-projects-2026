...
const statusName=s=>({Draft:'Utkast',Review:'Til vurdering',Approved:'Godkjent',Published:'Publisert',Open:'Åpen','In progress':'Pågår'}[s]||s);
...
function exportBacklog(){const rows=[['Prioritet','Tittel','Ressurser','Ansvarlig rolle','Status','Begrunnelse'],...state.backlog.map(x=>[priorityScore(x),x.title,x.assetIds.join('; '),x.ownerRole,statusName(x.status),x.reason])];download('metaready-tiltak.csv',rows.map(r=>r.map(csvCell).join(',')).join('\n'),'text/csv;charset=utf-8');toast('Tiltaksloggen er eksportert.');}
...
document.addEventListener('change',event=>{
  ...
  if(el.dataset.action==='backlog-status'){if(!require('remediate')){render();return;}const item=state.backlog.find(x=>x.id===el.dataset.id);const before=item.status;item.status=el.value;addAudit(`Endret tiltak: ${statusName(before)} → ${statusName(item.status)}`,item.title,`Tiltaksstatus oppdatert av ${roleName(state.role)}.`);save();toast('Tiltaksstatus oppdatert.');}
});
